...
- (void)didBeginLongPressGestureRecognizer:(UILongPressGestureRecognizer*)gestureRecognizer
{
    ...
    if (indexPath) {
        ...
        ColorShoppingCatrCell *cell = (ColorShoppingCatrCell *)[self.tableView cellForRowAtIndexPath:indexPath];
        ...
        if ([currentDictionary[@"orderCountTF"] integerValue] ==1 ) {
            self.identifier = nil;
            _canSplitCell = NO;
            NSLog(@"NO");
            
        }
        else if ([currentDictionary[@"orderCountTF"] integerValue] >1){
            self.identifier = currentDictionary[@"seka_id"];
            _canSplitCell = YES;
            
            NSInteger count = [currentDictionary[@"orderCountTF"] integerValue] -1;
            [currentDictionary setObject:[NSNumber numberWithInteger:count] forKey:@"orderCountTF"];
